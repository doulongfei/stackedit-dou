...
- 体验优化：新建文件/文件夹、搜索文件时输入框自动聚焦，提升操作效率（2025-11-28）

## 本地私有化部署与体验优化
本项目致力于提供一个**私有化、数据安全、体验更优**的 Markdown 编辑器解决方案：
- **私有化部署**：完全掌控您的数据，支持部署在个人服务器或内网环境，无需依赖外部云服务，保障数据隐私。
- **优化的使用体验**：针对国内用户习惯进行了深度优化，包括汉化界面、支持国内常用代码托管平台（Gitee、Gitea）、优化输入交互（如自动聚焦）等。
